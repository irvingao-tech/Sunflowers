...
export function addStudioLighting(scene,renderer,lowPower=false){
  ...
  if(lowPower){
    const fill=new THREE.DirectionalLight(0xd4e0ef,.5);fill.position.set(4,3,3);scene.add(fill);
    const rim=new THREE.DirectionalLight(0xffda9f,1.2);rim.position.set(2,5,-4);scene.add(rim);
  }else{
  ...
  const beam=new THREE.Mesh(new THREE.CylinderGeometry(.08,2.6,length,lowPower?12:40,1,true),new THREE.ShaderMaterial({
